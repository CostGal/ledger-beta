...
**Beta build:** `index.html` here was re-promoted from `ledger`'s current sandbox build (emoji icons, unsaved-write badge, remembered email, view/date persisted across re-auth, plus the earlier icon/manifest, `live()`/`render()` split, `pushWrite` serialisation, password reset, retry-on-reconnect) — this branch, pending merge to `main`. If friends report an old build after this merges, it's a GitHub Pages deploy/cache lag, not a stale repo.
...
- [x] [claude] Mirrored every In progress / Queued item above as a GitHub issue in `CostGal/ledger` (#3–#19) and noted the sync convention at the top of this file.
